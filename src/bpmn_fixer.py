...
class BpmnAutoFixer:
    ...
    def generate_fixed_bpmn(original_file_path: str, nodes: list[BpmnNode], output_path: str = "fixed_process.bpmn"):
        # Registriamo i namespace per non rovinare i tag XML originali
        ...
        for process in root.findall('bpmn:process', namespaces):
            for elem in process:
                ...
                if node_id in node_dict:
                    ...
                    if node.profile:
                        # --- REMEDIATION ETICA (Auto-Fix delle Regole) ---
                        ...
                        # Se è equità ma mancano i criteri definiti (Regola 1), li diamo per definiti
                        if node.profile.equity_action != EquityAction.NONE:
                            node.profile.criteria_defined = True

                        # --- MODIFICA DELL'XML ---
                        # Cerchiamo o creiamo il blocco <extensionElements>
                        ...
                        # Rimuoviamo il vecchio tag etico se esiste (per sovrascriverlo)
                        old_profile = ext_elem.find('{http://ethicbpmn.org/schema/1.0/ethic}TaskProfile')
                        if old_profile is not None:
                            ext_elem.remove(old_profile)

                        # Creiamo il nuovo tag XML con i parametri corretti
